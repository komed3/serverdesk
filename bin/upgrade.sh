...
echo -e "${YELLOW}=== Step 1: Update package sources ===${RESET}"
apt update

countdown 0

# Step 2 - Show list of upgradable packages
echo -e "${YELLOW}=== Step 2: Packages that need to be updated ===${RESET}"
...
# Step 3 – Upgrade
echo -e "${YELLOW}=== Step 3: Upgrade installed packages ===${RESET}"
apt upgrade -y

countdown 0

# Step 4 – Distribution upgrade
echo -e "${YELLOW}=== Step 4: System upgrade (distribution) ===${RESET}"
apt dist-upgrade -y

countdown 5

# Step 5 – Autoremove not used packages
echo -e "${YELLOW}=== Step 5: Remove packages no longer needed ===${RESET}"
apt autoremove -y

countdown 0

# Step 6 – Clean up package cache
echo -e "${YELLOW}=== Step 6: Clean up package cache ===${RESET}"
apt clean
